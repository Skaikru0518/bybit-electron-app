    ...
    <div className="bg-card-bg p-6 rounded-lg shadow-lg mb-8 flex flex-col w-[400px]">
      ...
      {averageEntryPrice && (
        <p className="mt-4 text-center text-white">
          New Average Entry Price: {averageEntryPrice} USDT
        </p>
      )}
    </div>
  );
...
const TPSLCalculator = () => {
  ...
  const calculateTPSL = () => {
    ...
    if (takeProfitPercent) {
      tpPrice =
        parseFloat(entryPrice) * (1 + parseFloat(takeProfitPercent) / 100);
      slPrice =
        parseFloat(entryPrice) *
        (1 - parseFloat(takeProfitPercent) / riskRewardRatio / 100);
    } else if (stopLossPercent) {
      slPrice =
        parseFloat(entryPrice) * (1 - parseFloat(stopLossPercent) / 100);
      tpPrice =
        parseFloat(entryPrice) *
        (1 + (parseFloat(stopLossPercent) * riskRewardRatio) / 100);
    }
  ...
  return (
    <div className="bg-card-bg p-6 rounded-lg shadow-lg mb-8 flex flex-col w-[400px]">
      ...
      <p className="text-sm text-center text-muted-text mb-4">
        Either TP or SL is required to be filled, the other can be calculated
        from RR.
      </p>
      <button
...
const Calculator = () => {
  ...
  // Navigációs gombok konfigurációja
  const calculatorButtons = [
    { id: 'entry', label: 'Entry Calculator' },
    { id: 'tpsl', label: 'TP/SL Calculator' },
    { id: 'order', label: 'Order by Value Calculator' },
  ];

  return (
    <div className="text-primary-text p-6 w-full min-h-[calc(100vh-60px)] flex flex-col items-center">
      <motion.h1
      ...
      <div className="calculator-nav flex justify-center space-x-4 mb-8">
        {calculatorButtons.map((button) => (
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className={`p-3 ${
              activeCalculator === button.id ? 'bg-btn' : 'bg-gray-800'
            } rounded-lg text-primary-text hover:bg-btn-hover transition-all`}
            onClick={() => setActiveCalculator(button.id)}
          >
            {button.label}
          </motion.button>
        ))}
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          transition={{
            duration: 0.3,
            ease: [0.43, 0.13, 0.23, 0.96], // Egyedi easing a simább animációért
          }}
