...
function Settings() {
    ...
    const handleSaveInterval = () => {
        ...
        if ([5000, 10000, 30000].includes(refreshInterval)) {
            ...
            setErrorMessage('Invalid interval value. Please choose from 5000ms, 10000ms, or 30000ms.');
            setTimeout(() => setErrorMessage(''), 3000); // Hide after 3 seconds
        }
    };
        ...
        <div className="w-full min-h-[calc(100vh-60px)] p-6 bg-gradient-to-r from-[#130f40] to-black text-primary-text flex flex-col items-center">
            ...
            <div className='flex flex-col w-[400px] justify-center'>
                <div className="space-y-4">
                    {/* API Key Input */}
                    <div>
                        <label htmlFor="apiKey" className="block text-sm text-gray-400 mb-2">API Key</label>
                        <input
                            type="text"
                            id="apiKey"
                            value={apiKey}
                    ...
                    {/* API Secret Input */}
                    <div>
                        <label htmlFor="apiSecret" className="block text-sm text-gray-400 mb-2">API Secret</label>
                        <input
                            type="password"
                            id="apiSecret"
                            value={apiSecret}
                ...
                {/* Refresh Interval */}
                <div className="mt-6">
                    <label htmlFor="refreshInterval" className="block text-sm text-gray-400 mb-2">Refresh Interval (ms)</label>
                    <select
                        id="refreshInterval"
                    ...
                    <p className="mt-2 text-sm text-gray-400">Current Interval: {refreshInterval} ms</p>
                </div>
