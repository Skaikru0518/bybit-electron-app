...
import TPSLPopup from './popups/TPSLPopup';
...
export default function Trades() {
  ...
  return (
    <div className="w-full min-h-[calc(100vh-60px)] p-6 text-primary-text">
      <div>
        ...
        {isOrderPopupOpen && (
          <OrderPopup onClose={() => setIsOrderPopupOpen(false)} />
        )}
        {isTPSLPopupOpen && (
          <TPSLPopup
        ...
        <div className="overflow-x-auto shadow-md">
          <table className="w-full border border-border text-xs rounded-lg shadow">
            <thead className="text-gray-300 uppercase text-xs bg-navbar-bg">
              <tr>
            ...
            <tbody>
              {positions.length > 0 ? (
                positions.map((position, index) => (
                  <tr
                    key={index}
                    className="border-b border-border text-center  hover:bg-table-hover transition-all duration-200 ease-in-out cursor-pointer"
                  >
                    ...
                    <td
                      className={
                        position.curRealisedPnl > 0
                          ? 'text-green-500'
                          : 'text-red-500'
                      }
                    >
                      {formatNumber(position.curRealisedPnl)}
                    </td>
                    <td>
                      {position.takeProfit > 0
                        ? formatNumber(position.takeProfit) + ' USDT'
                        : '/'}
                    </td>
                    <td>
                      {position.stopLoss > 0
                        ? formatNumber(position.stopLoss) + ' USDT'
                        : '/'}
                    </td>
                    <td>
                      <button
                ...
                <tr>
                  <td colSpan="13" className="p-4 text-center text-muted-text">
                    No open positions.
                  </td>
                </tr>
              )}
      ...
      <div className="overflow-x-auto flex flex-col">
        <h2 className="text-center text-2xl font-semibold mb-4 mt-12">
          Pending Orders
        </h2>
        <table className="w-full border border-border text-xs rounded-lg shadow">
          <thead>
            ...
            {pendingOrders.length > 0 ? (
              pendingOrders.map((order, index) => (
                ...
                >
                  ...
                  <td
                    className="p-2"
                    style={{ color: order.side === 'Buy' ? 'green' : 'red' }}
                  >
                    {order.side}
                  </td>
                  ...
                  <td className="p-2">
                    <button
                      className="bg-[#6b05de] text-[#e0e0e0] border border-[#3b3663] px-4 py-2 rounded-md font-semibold text-sm transition-transform duration-200 ease-in-out hover:bg-[#5a04c7] hover:shadow-lg hover:scale-105 active:bg- disabled:bg-[#555555] disabled:opacity-60 disabled:cursor-not-allowed"
                      onClick={() =>
              ...
              <tr>
                <td colSpan="7" className="text-center p-4 text-muted-text">
                  No pending orders.
                </td>
              </tr>
            )}
